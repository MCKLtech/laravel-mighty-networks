...
/**
 * `GET networks/{network_id}/plans/{plan_id}/members`
 *
 * Paginated: `page` (default 1) and `per_page` (default 25, max 100).
 */
final class ListPlanMembersRequest extends AdminRequest implements MapPaginatedResponseItems, Paginatable
{
     ...
     */
    #[\Override]
    public function createDtoFromResponse(Response $response): MemberCollection
    {
        return new MemberCollection($this->mapPaginatedResponseItems($response));
    }

    /**
     * Map each raw item to a {@see Member}, so paginators yield DTOs.
     *
    ...
    {
        return array_map(
            static fn (mixed $item): Member => Member::fromArray(is_array($item) ? $item : []),
