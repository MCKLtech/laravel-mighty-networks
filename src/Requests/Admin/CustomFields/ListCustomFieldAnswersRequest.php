...
/**
 * `GET networks/{network_id}/custom_fields/{custom_field_id}/members/{member_id}/answers`
 *
 * Paginated: `page` (default 1) and `per_page` (default 25, max 100).
 */
final class ListCustomFieldAnswersRequest extends AdminRequest implements MapPaginatedResponseItems, Paginatable
{
     ...
     */
    #[\Override]
    public function createDtoFromResponse(Response $response): CustomFieldAnswerCollection
    {
        return new CustomFieldAnswerCollection($this->mapPaginatedResponseItems($response));
    }

    /**
     * Map each raw item to a {@see CustomFieldAnswer}, so paginators yield DTOs.
     *
     * @return array<int, CustomFieldAnswer>
     */
    #[\Override]
    public function mapPaginatedResponseItems(Response $response): array
    {
        return array_map(
            static fn (mixed $item): CustomFieldAnswer => CustomFieldAnswer::fromArray(is_array($item) ? $item : []),
