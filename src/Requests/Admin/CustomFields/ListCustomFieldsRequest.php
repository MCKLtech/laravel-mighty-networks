...
/**
 * `GET networks/{network_id}/custom_fields`
 *
 * Optionally filtered by `response_type`; paginated with `page` (default 1)
 * and `per_page` (default 25, max 100).
 */
final class ListCustomFieldsRequest extends AdminRequest implements MapPaginatedResponseItems, Paginatable
{
    protected Method $method = Method::GET;

    public function __construct(
        int|string $networkId,
        protected CustomFieldResponseType|string|null $responseType = null,
     ...
     */
    #[\Override]
    public function createDtoFromResponse(Response $response): CustomFieldCollection
    {
        return new CustomFieldCollection($this->mapPaginatedResponseItems($response));
    }

    /**
     * Map each raw item to a {@see CustomField}, so paginators yield DTOs.
     *
    ...
    {
        return array_map(
            static fn (mixed $item): CustomField => CustomField::fromArray(is_array($item) ? $item : []),
