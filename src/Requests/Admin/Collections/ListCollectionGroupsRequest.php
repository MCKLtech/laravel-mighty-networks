...
/**
 * `GET networks/{network_id}/collections`
 *
 * Paginated: `page` (default 1) and `per_page` (default 25, max 100).
 */
final class ListCollectionGroupsRequest extends AdminRequest implements MapPaginatedResponseItems, Paginatable
{
     ...
     */
    #[\Override]
    public function createDtoFromResponse(Response $response): CollectionGroupCollection
    {
        return new CollectionGroupCollection($this->mapPaginatedResponseItems($response));
    }

    /**
     * Map each raw item to a {@see CollectionGroup}, so paginators yield DTOs.
     *
     * @return array<int, CollectionGroup>
     */
    #[\Override]
    public function mapPaginatedResponseItems(Response $response): array
    {
        return array_map(
            static fn (mixed $item): CollectionGroup => CollectionGroup::fromArray(is_array($item) ? $item : []),
