...
/**
 * `GET networks/{network_id}/members/{member_id}/tags`
 *
 * Paginated: `page` (default 1) and `per_page` (default 25, max 100).
 */
final class ListMemberTagsRequest extends AdminRequest implements MapPaginatedResponseItems, Paginatable
{
     ...
     */
    #[\Override]
    public function createDtoFromResponse(Response $response): TagCollection
    {
        return new TagCollection($this->mapPaginatedResponseItems($response));
    }

    /**
     * Map each raw item to a {@see Tag}, so paginators yield DTOs.
     *
    ...
    {
        return array_map(
            static fn (mixed $item): Tag => Tag::fromArray(is_array($item) ? $item : []),
